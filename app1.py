...
# Load the YOLO modelSS
model_path ="C:\\Users\\akhil\\OneDrive\\Documents\\flask_projects\\yolo_black_grains_flask_app\\best_whaet_model_train2.pt"
model = YOLO(model_path)

# Set default confidence threshold
CONF_THRESHOLD = 0.305  # Modify as per your requirement
...
def predict():
    ...
    if 'threshold' in request.form:
        try:
            CONF_THRESHOLD = float(request.form['threshold'])
        except ValueError:
            return render_template('result.html', error="Invalid threshold value provided", class_counts=None, result_image=None, class_mapping=None)
    ...
    # Run prediction with the set confidence threshold
    ...
    for result in results:
        for box in result.boxes:
            ...
            # Add text label
            text = f"ID: {class_id} ({box.conf.item():.2f})"
    ...
    return render_template('result.html', error=None, class_counts=class_counts, result_image=img_str, class_mapping=class_mapping, threshold=CONF_THRESHOLD)
